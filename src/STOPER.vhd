...
entity Stoper is
    Port ( 
        clk           : in  STD_LOGIC;                    -- sygnal zegarowy (50MHz)
        rst           : in  STD_LOGIC;                    -- sygnal resetu
        start_pause   : in  STD_LOGIC;                    -- przycisk start/pauza
        save          : in  STD_LOGIC;                    -- przycisk do zapisu wyniku
        memory_select : in  STD_LOGIC_VECTOR(1 downto 0); -- wybór pamieci do wyœwietlenia (2 bity)
        minutes       : out INTEGER range 0 to 59;        -- wyœwietlane minuty
        seconds       : out INTEGER range 0 to 59;        -- wyœwietlane sekundy
        memory_valid  : out STD_LOGIC_VECTOR(2 downto 0)  -- flagi waznosci zapisanych czasów (czy czas jest aktualny i dobrze zapisany)
    );
end Stoper;
...
architecture Behavioral of Stoper is
    ...
    -- stale
    constant CLK_FREQ : integer := 50_000_000; -- 50MHz (jedna pelna operacja zajmuje 20ns)
    ...
    type stored_time is record
        ...
        valid   : std_logic;					-- flaga waznosci danych
    end record;
    ...
    -- sygnaly wewnetrzne
    signal running       : std_logic := '0';  								-- status dzialania stopera
    ...
    signal memory_index  : integer range 0 to 2 := 0;						-- indeks do zapisu pamiêci (wskazuje, która pamiêæ jest aktualnie uzywana do zapisu)
...
begin
	 -- glówny proces sterujacy dzialanie stopera
    process(clk, rst)
    ...
    begin
        if rst = '1' then
            ...
            -- obsluga przycisku start/pauza
            if start_pause = '1' then
                if running = '0' and paused = '0' then
                    running <= '1';		-- rozpoczêcie dzialania stopera
            ...
            -- zliczanie czasu gdy stoper dziala
            if running = '1' then
                if clk_counter = CLK_FREQ-1 then
                    ...
                    if sec_count = 59 then
                        ...
                        if min_count = 59 then
                            min_count <= 0;		-- przepenienie minut
                        else
                            min_count <= min_count + 1;	-- inkrementacja minut (uplyw 59 sekund)
                        end if;
                    else
            ...
            -- obsluga zapisu czasu
            if save = '1' and memory_index < 3 then
                memory(memory_index) <= (min_count, sec_count, '1');	-- zapis czasu do pamieci, '1' -> valid - wa¿ny zapis
    ...
    begin
        case memory_select is
            when "00" =>	-- wyœwietlanie biezacego czasu
    ...
    seconds <= display_secs;
	 -- flaga valid wskazuje, czy zapisany czas jest wazny
